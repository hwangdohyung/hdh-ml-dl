...
h1 = tf.matmul(x, w1) + b1    # 여기서 나온 h와 아웃풋 레이어와 연산됨.

## hidden layer 2##
w2 = tf.Variable(tf.random_normal([20, 10], name='weights'))
b2 = tf.Variable(tf.random_normal([10], name='bias'))

h2 = tf.sigmoid(tf.matmul(h1, w2) + b2)

## hidden layer 3##
w3 = tf.Variable(tf.random_normal([10, 5], name='weights'))
b3 = tf.Variable(tf.random_normal([5], name='bias'))

h3 = tf.matmul(h2, w3) + b3

## hidden layer 4##
w4 = tf.Variable(tf.random_normal([5, 1], name='weights'))
b4 = tf.Variable(tf.random_normal([1], name='bias'))

h = tf.sigmoid(tf.matmul(h3, w4) + b4)  
