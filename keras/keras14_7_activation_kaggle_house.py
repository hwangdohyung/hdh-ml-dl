...
#earlystopping을 했을 때 overfitting을 방지할 수 있었고 성능향상이 되었다, 또 낭비없이 적절한 훈련랑을 조절 할 수 있엇다. 
# patience를 일정이상 올렸을 때는 큰 의미가 없어졌다.
# restore best weight를 통해 최적의 weight값으로 훈련을 정지할 수 있었다. 
# relu 함수를 사용했을 때 대체로 성능이 향상되었고 
# 회귀모델에서는 sigmoid함수와 binary_crossentropy,accuracy등은 사용하기 힘들었다. 